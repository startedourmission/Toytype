...
test('실데이터: 합의 표기(macOS·JSON 형식·메타데이터 등)로 통일', () => {
  initAll();
  const stay = [
    'macOS에서 실행한다',
    'JSON 형식으로 저장한다',
  ...
  for (const text of stay) {
    assert.equal(E.scan(text).findings.length, 0, `정상 표기 오검출: ${text}`);
  }

  const cases = [
    ['맥 OS X에서 실행한다', '맥 OS X', 'macOS'],
    ['json형태로 저장한다', 'json형태', 'JSON 형식'],
    ['메타데이타를 읽는다', '메타데이타', '메타데이터'],
    ['라이프사이클을 따른다', '라이프사이클', '생명주기'],
    ['좀더 자세히 본다', '좀더', '더'],
    ['좀 더 자세히 본다', '좀 더', '더'],
    ['커멘드 라인을 연다', '커멘드 라인', '커맨드라인'],
    ['아래코드를 본다', '아래코드', '다음 코드'],
    ['기능이 존재 합니다', '존재 합니다', '있습니다'],
    ['타깃 값을 계산한다', '타깃 값', '목푯값'],
    ['타겟 변수를 고른다', '타겟 변수', '타깃 변수'],
    ['화상채팅을 한다', '화상채팅', '화상 채팅'],
    ['http요청을 보낸다', 'http요청', 'HTTP 요청'],
    ['이런경우 다시 시도한다', '이런경우', '이런 경우'],
    ['해 주어야 한다', '해 주어야', '해야'],
    ['할 수 있게 해주는 기능', '할 수 있게 해주는', '하는']
  ];
  for (const [text, src, dst] of cases) {
    const f = E.scan(text).findings.find((x) => x.src === src);
    assert.ok(f, `'${src}' 미검출`);
    assert.equal(f.dst, dst, `'${src}' 교정값`);
  }
});
